    ...
    <x-auth-card>
        ...
        <x-splade-form action="{{ route('login') }}" class="space-y-4">
            ...
            <div class="flex items-center justify-end">
                ...
                <Link class="text-align:left underline text-sm text-gray-600 hover:text-gray-900" href="{{ route('usercredentials.index') }}">
                        {{ __('Migrate Credentials?') }}
                    </Link>
                  

                @if (Route::has('password.request'))
                    <Link class="underline text-sm ml-6  text-gray-600 hover:text-gray-900" href="{{ route('password.request') }}">
